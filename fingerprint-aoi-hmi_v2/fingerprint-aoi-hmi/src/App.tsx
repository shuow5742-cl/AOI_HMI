...
import {
  ...
  getRobotCurrentPoseFromBackend,
  RobotStatus,
...
function App() {
  ...
  const handleCapturePose = async (key: keyof RecipeDbRow) => {
    if (!editRow) {
      setCommInfo((prev) => ({
        ...prev,
        lastReceivedMessage: "读取失败：当前没有可编辑的型号",
      }));
      return;
    }
    ...
    if (robotMode !== "real") {
      setCommInfo((prev) => ({
        ...prev,
        lastReceivedMessage: "读取失败：当前为 mock 模式，请先切换到真实模式",
      }));
      return;
    }
    ...
    try {
      setCommInfo((prev) => ({
        ...prev,
        lastReceivedMessage: `正在读取机械臂当前位置并写入 ${String(key)}...`,
      }));

      const result = await getRobotCurrentPoseFromBackend();

      if (!result.ok || !result.poseText) {
        setCommInfo((prev) => ({
          ...prev,
          lastReceivedMessage: result.message || "读取机械臂当前位置失败",
        }));
        return;
      }
      ...
      setEditRow((prev) => {
        if (!prev) return prev;
        return { ...prev, [key]: result.poseText! };
      });

      setCommInfo((prev) => ({
        ...prev,
        lastReceivedMessage: `当前位置已写入 ${String(key)} -> ${result.poseText}`,
      }));

      setRuntimeInfo((prev) => ({
        ...prev,
        lastAction: `已读取机械臂当前位置并写入 ${String(key)}`,
      }));
    } catch (error) {
      setCommInfo((prev) => ({
        ...prev,
        lastReceivedMessage: `读取机械臂当前位置失败：${String(error)}`,
      }));
    }
  };
