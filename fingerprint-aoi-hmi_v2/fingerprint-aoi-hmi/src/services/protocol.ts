...
function buildKeyValueMessage(key: string, value: string | number) {
  return JSON.stringify({
    key,
    value,
  });
...
function appendPoseMessages(queue: string[], pointName: string, poseText: string) {
  ...
  AXES.forEach((axis, index) => {
    queue.push(
      buildKeyValueMessage(
        `${pointName}_${axis}`,
        Number(poseValues[index] ?? 0)
      )
    );
  });
}
...
export function buildRecipeMessageQueue(recipe: RecipeDbRow): string[] {
  ...
  queue.push(buildKeyValueMessage("flag_ramp", Number(recipe.flag_ramp ?? 0)));
  queue.push(buildKeyValueMessage("row_n", Number(recipe.row_n ?? 0)));
  queue.push(buildKeyValueMessage("col_n", Number(recipe.col_n ?? 0)));
  queue.push(buildKeyValueMessage("dz_check", Number(recipe.dz_check ?? 0)));
